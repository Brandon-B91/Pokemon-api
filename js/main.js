...
function setQuery(e) {
  if (e.keyCode == 13 || e.keyCode == 36) {
    ...
    if(searchBox.value == '') {
        alert("please enter a number between 1-898 or a pokemon name I.E pikachu")
        e.preventDefault()
    }
    searchBox.value = "";
  }
...
function displayPokemonName(pokemon) {
  ...
  if (pokemon.types[0].type.name === "electric") {
    card.style.background =
      "linear-gradient(to bottom right, lightyellow, yellow)";
  ...
  if (pokemon.types[0].type.name === "fighting") {
    card.style.background = "linear-gradient(to bottom right, pink, salmon)";
    typeImg.src = "types/fighting.svg";
  }
  ...
  if (
    pokemon.types[0].type.name === "normal" && pokemon.types[1].type.name === "flying") {
    card.style.background =
      "linear-gradient(to bottom right, white, lightgray)";
  ...
  if (pokemon.types[0].type.name === "ice") {
    card.style.background =
      "linear-gradient(to bottom right, white, lightblue)";
  ...
  if (pokemon.types[0].type.name === "steel") {
    card.style.background =
      "linear-gradient(to bottom right, lightgray, silver)";
...
function displayWeakness(pokemon) {
  ...
  if(pokemon.damage_relations.half_damage_from === 'undefined') {
      alert('you suck')
...
function displaySpecies(species) {
  ...
  dexEntry.innerHTML = species.flavor_text_entries[1].flavor_text;
}
